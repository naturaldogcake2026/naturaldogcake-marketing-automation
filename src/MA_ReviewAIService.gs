...
function ma_aiReviewCallOpenAI_(ctx) {
  ...
  var instructions = [
    ...
    '1-1. 설정 시트 운영 사실과 콘텐츠 기획의 사실확인메모에 명시되지 않은 주문 절차·접수 방식·사진 제출 규칙·사진 장수·우선 사진 지정·옵션·제작 가능 범위를 사실이나 지시사항처럼 새로 만들지 않는다.',
    '1-2. 원문에 위와 같은 추론성 운영 안내가 있으면 삭제하거나, 필요한 경우 구체적 절차를 만들지 말고 단순히 주문 전 문의로 확인하도록 바꾼다.',
    '1-3. 특히 “여러 장의 사진 중 우선 사진을 지정해 달라”, “이 정보를 함께 보내 달라”처럼 설정에 없는 고객 행동을 요구하는 문장을 만들거나 유지하지 않는다.',
